...
async function seed() {
    ...
    // --- Test 2: Normalization (open_text + fill_blank + single) ---
    const q2a = uuidv4(); // open_text
    db.prepare(`INSERT INTO questions (id,test_id,text,type,weight,sort_order,created_at) VALUES (?,?,?,?,?,?,?)`).run(
        q2a, test2, 'Дайте определение первой нормальной формы (1НФ).', 'open_text', 3, 1, ago(38));

    const q2b = uuidv4(); // open_text
    db.prepare(`INSERT INTO questions (id,test_id,text,type,weight,sort_order,created_at) VALUES (?,?,?,?,?,?,?)`).run(
        q2b, test2, 'Приведите пример таблицы, нарушающей 2НФ, и покажите как её нормализовать.', 'open_text', 5, 2, ago(38));
    ...
    const q2d = uuidv4(); // fill_blank
    db.prepare(`INSERT INTO questions (id,test_id,text,type,weight,sort_order,created_at) VALUES (?,?,?,?,?,?,?)`).run(
        q2d, test2, 'Процесс приведения таблиц к нормальным формам называется ________.', 'fill_blank', 2, 4, ago(38));
    [[uuidv4(),'нормализация',1],[uuidv4(),'нормализацией',1]].forEach(([id,text,cor]) =>
        db.prepare(`INSERT INTO answers (id,question_id,text,is_correct,position) VALUES (?,?,?,?,?)`).run(id, q2d, text, cor, null));
    ...
    const q4c = uuidv4(); // open_text
    db.prepare(`INSERT INTO questions (id,test_id,text,type,weight,sort_order,created_at) VALUES (?,?,?,?,?,?,?)`).run(
        q4c, test4, 'Объясните разницу между SIGTERM и SIGKILL.', 'open_text', 3, 3, ago(30));

    const q4e = uuidv4(); // fill_blank
    db.prepare(`INSERT INTO questions (id,test_id,text,type,weight,sort_order,created_at) VALUES (?,?,?,?,?,?,?)`).run(
        q4e, test4, 'Команда для просмотра запущенных процессов в реальном времени: ________.', 'fill_blank', 1, 4, ago(30));
    [[uuidv4(),'top',1],[uuidv4(),'htop',1]].forEach(([id,text,cor]) =>
        db.prepare(`INSERT INTO answers (id,question_id,text,is_correct,position) VALUES (?,?,?,?,?)`).run(id, q4e, text, cor, null));

    const q4d = uuidv4(); // match
    db.prepare(`INSERT INTO questions (id,test_id,text,type,weight,sort_order,created_at) VALUES (?,?,?,?,?,?,?)`).run(
        q4d, test4, 'Сопоставьте команды с их назначением:', 'match', 2, 5, ago(30));
    const pairs2 = [
    ...
    console.log('Типы вопросов: single, multiple, open_text, fill_blank, match, order');
